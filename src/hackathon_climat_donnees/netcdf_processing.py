...
# 0. Paramètres
# -----------------------------------------------
PATH_ROOT = "/home/azureuser/HACKATHON"
PATH_CSV  = "/home/azureuser/HACKATHON"
PATH_DATA = "/home/azureuser/"
PATH_OUT = "/home/azureuser/HACKATHON/out"
...
# -----------------------------------------------
with open(os.path.join(PATH_ROOT, "liste_ssp370_tasmax.txt")) as f:
    ssp_files = [l.strip() for l in f.readlines()]

with open(os.path.join(PATH_ROOT, "liste_hist_tasmax.txt")) as f:
...
df = pd.read_csv(os.path.join(PATH_ROOT,"TRACC_pivot.csv"))
...
for RWL in ["2C","2.7C","4C"]:
    ...
    for _, row in df.iterrows():
        ...
        if any(model_key in s for s in mods):
            ...
            ds_hist_sel   = ds_hist.sel(time=slice(f'{start}-01-01', f'{end}-12-31'))
    ...
    if mods and not HIST_temp:
        ...
        HIST = HIST.median(dim='modele').to_netcdf(os.ath.join(PATH_OUT, file_name_hist))

        file_name_hist = f"{VAR}_RP_hist_ref_sup.nc"
        HIST = xr.concat(HIST_temp, dim='modele')
        HIST = HIST.quantile(0.95,dim='modele').to_netcdf(os.ath.join(PATH_OUT, file_name_hist))

        file_name_hist = f"{VAR}_RP_hist_ref_inf.nc"
        HIST = xr.concat(HIST_temp, dim='modele')
        HIST = HIST.quantile(0.05,dim='modele').to_netcdf(os.ath.join(PATH_OUT, file_name_hist))


    file_name_rcp8 = f"{VAR}_RP_ssp3_+{RWL}_median.nc"
    RCP8 = xr.concat(RCP8_temp, dim='modele')
    RCP8 = RCP8.median(dim='modele').to_netcdf(os.ath.join(PATH_OUT, file_name_rcp8))
    
    file_name_rcp8 = f"{VAR}_RP_ssp3_+{RWL}_sup.nc"
    RCP8 = xr.concat(RCP8_temp, dim='modele')
    RCP8 = RCP8.quantile(0.95,dim='modele').to_netcdf(os.ath.join(PATH_OUT, file_name_rcp8))

    file_name_rcp8 = f"{VAR}_RP_ssp3_+{RWL}_inf.nc"
    RCP8 = xr.concat(RCP8_temp, dim='modele')
    RCP8 = RCP8.quantile(0.05,dim='modele').to_netcdf(os.ath.join(PATH_OUT, file_name_rcp8))
